...
function collectCandidateFiles(options: CliOptions): string[] {
  ...
  const result = spawnSync('rg', ['--files', 'tests'], {
    cwd: repoRoot,
    encoding: 'utf8',
  });
...
function printSummary(runRecords: RunRecord[], heuristics: Heuristic[]): boolean {
...
function main(): void {
  ...
  const foundIntermittentFailures = printSummary(runRecords, heuristics);
  process.exitCode = foundIntermittentFailures ? 1 : 0;
}
